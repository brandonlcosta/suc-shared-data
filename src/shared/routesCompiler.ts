...
import { SHARED_DATA_ROOT, getEventId, getRouteGroupIds } from './canonical';
...
const ROUTE_COLOR_BY_LABEL: Record<RouteLabel, string> = {
  MED: '#00FF99',
  LRG: '#13FFE2',
  XL: '#D000FF',
  XXL: '#FF5050',
...
function parseGpxPoints(raw: string) {
  if (!raw) return [];
...
function buildGeoJson(points: Array<{ lat: number; lon: number }>, poiFeatures: unknown[] = []) {
...
function computeStats(points: Array<{ lat: number; lon: number; ele: number | null }>) {
  if (points.length === 0) {
    return {
      distanceMeters: 0,
...
async function readRouteMeta(routeGroupId: string) {
  ...
  return meta as { variants: string[] };
}

async function readRoutePois(routeGroupId: string) {
...
async function readGpx(routeGroupId: string, routeId: string, label: string) {
...
function normalizePoiVariant(raw: any) {
  if (!raw) return null;
...
function compilePoisForVariant(poisDoc: { pois: any[] } | null, label: string) {
...
export async function compileRouteGroup(routeGroupId: string, options: UrlOptions = {}) {
  ...
  const compiled: CompiledRouteEntry[] = [];
  for (const rawLabel of variants) {
    const label = normalizeVariantKey(rawLabel) as RouteLabel;
...
export async function compileRoutesForEvents(events: unknown[], options: UrlOptions = {}) {
  if (!Array.isArray(events)) {
    throw new Error('[BROADCAST] compileRoutesForEvents expected an array of events.');
  }

  const compiledRoutes: CompiledRoutesIndex = { routes: {} };
  const routes: Array<{
    route_id: string;
    event_id: string;
    label: RouteLabel;
  ...
  const routeGroupCache = new Map<string, CompiledRouteEntry[]>();
